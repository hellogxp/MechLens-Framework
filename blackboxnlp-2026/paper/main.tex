...
\begin{abstract}
Vocabulary projections are often used to ask when an answer becomes visible inside a language model. A common scalar summary---the first layer at which a target enters the vocabulary top-$k$---has two failure modes: a target that never enters top-$k$ can be conflated with final-layer entry, and a transient entry can be mistaken for stable readout. We audit this measurement and introduce a trajectory-aware protocol that separates first entry, persistent entry, later dropout, and right-censored non-entry. Across 817 TruthfulQA questions and six 7--14B base models, 21.9--31.1\% of target tokens never enter the top-10, while 15.2--48.0\% enter and later leave it. The scalar first-entry depth hides qualitatively different dynamics: Pythia enters early but is unstable, whereas Gemma enters late and stabilizes almost exclusively at the output. Controls on MMLU and SST-2 further show strong task-format dependence. On SST-2, Mistral and Pythia reach 86.9\% and 80.8\% candidate-choice accuracy while the correct label appears in the final full-vocabulary top-10 for 0\% of examples. We therefore argue that layerwise vocabulary rank is a property of a model--prompt--target interface, not a direct measure of knowledge storage, causal use, or even candidate-task competence. We release corrected code, per-layer ranks, audit checks, and tests in \mechlens{}.
\end{abstract}
...
We present a measurement audit and a corrected trajectory protocol. We retain the useful question behind the Factual Emergence Point (FEP)---when does a specified target first become directly readable through the model's native output head?---but explicitly distinguish four outcomes: first entry, persistent entry, dropout after entry, and never observed. We also extract the target by jointly tokenizing prompt and continuation, avoiding tokenizer-boundary changes.

Our experiments reveal a more heterogeneous picture than a single ``late emergence'' statistic suggests. On TruthfulQA, final top-10 visibility is similar across six models (64.7--71.7\%), yet the paths differ sharply. Pythia-6.9B first exposes observed targets at 45.4\% depth on average but exhibits 28.4\% dropout; Gemma-7B first exposes them at 84.9\% depth and exhibits 48.0\% dropout, with persistent entry at 100.0\% depth. Qwen2.5-7B and 14B show similar normalized profiles, but this within-family comparison does not identify a scaling law.
...
The logit lens applies the output unembedding to intermediate states \citep{nostalgebraist2020logitlens}. \citet{geva2022transformer} study how feed-forward updates promote vocabulary concepts, and \citet{belrose2023tuned} learn layer-specific affine translators to reduce distribution shift between intermediate and final representations. The tuned lens is more predictive than the raw logit lens, but both remain vocabulary-space probes rather than localizers of stored knowledge. Recent analyses also emphasize that projection behavior can be non-monotone or depend on spectral components poorly represented by a simple token ranking \citep{cancedda2024spectral,nguyen2026iglens}. Our focus is complementary: we audit the event statistic computed from such projections.
...
\end{align}
$F_i$ is undefined when the target never enters top-$k$; $P_i$ is defined only when it is present at the final layer. We additionally report $N_i=\mathbb{1}[\sum_l b_{i,l}=0]$ and dropout $D_i$, which is one when an observed target later leaves top-$k$. Means of $F_i/L$ are conditional on observed entries; means of $P_i/L$ are conditional on final presence. We always report their denominators alongside unconditional never-observed, final-presence, and dropout rates.
...
\toprule
Model & $L$ & Ever $\uparrow$ & Never $\downarrow$ & Final $\uparrow$ & Dropout $\downarrow$ & First / Persistent depth \\
\midrule
Qwen2.5-7B  & 28 & 68.9 & 31.1 & 64.7 & 18.8 & .824 / .876 \\
Qwen2.5-14B & 48 & 71.2 & 28.8 & 67.7 & 16.8 & .872 / .900 \\
Llama-3.1-8B & 32 & 73.8 & 26.2 & 71.7 & 15.2 & .833 / .865 \\
Mistral-7B & 32 & 73.2 & 26.8 & 67.9 & 18.5 & .788 / .835 \\
Pythia-6.9B & 32 & 78.1 & 21.9 & 71.1 & 28.4 & .454 / .615 \\
Gemma-7B & 28 & 76.5 & 23.5 & 71.7 & 48.0 & .849 / 1.000 \\
\bottomrule
\end{tabular}
\caption{Corrected top-10 trajectories on 817 TruthfulQA examples. Percentages use all examples. First depth is conditional on ever-observed targets; persistent depth is conditional on final top-10 presence. Wilson 95\% intervals for the Never rates are at most $\pm3.3$ points and are given in the released table.}
\label{tab:truthful}
\end{table*}
...
\caption{TruthfulQA top-10 trajectories. Left: fraction of target tokens in the vocabulary top-10 at each normalized depth. Right: right-censoring and non-monotonicity are both substantial. Pythia's early visibility plateau and Gemma's late collapse show why first entry alone is insufficient.}
\label{fig:truthful-trajectories}
\end{figure*}

Table~\ref{tab:truthful} replaces a single ``late'' rate with distinct events. Final visibility varies by only seven points across models, but first-entry depth varies from .454 to .872. Figure~\ref{fig:truthful-trajectories} explains this discrepancy.

\paragraph{Early does not mean stable.}
Pythia has the earliest conditional first entry (14.53/32 layers), yet 28.4\% of all targets drop out after entry and persistent entry moves to 19.67/32. Gemma is more extreme in the opposite direction: observed targets first enter at 23.77/28, 48.0\% of all targets drop out at least once, and final-presence cases become persistent at 27.99/28 on average. A scalar first entry calls both targets ``observed'' while missing their different dynamics.
...
Qwen2.5-7B and 14B have similar ever-observed rates (68.9\% vs. 71.2\%) and persistent depths (.876 vs. .900). With only two sizes, this is evidence of profile similarity, not scale invariance.
...
\begin{tabular}{lrrrr}
\toprule
Template & Ever & Final & Dropout & First / Persist. \\
\midrule
...
Qwen2.5-7B reaches 72.0\% candidate accuracy on 1,200 MMLU examples. Every correct A/B/C/D label both enters and ends in the full-vocabulary top-10. Conditional first and persistent depths are .646 and .731, respectively. Across STEM, humanities, social sciences, and other subjects, final visibility remains 100\% while candidate accuracy ranges from 63.1\% to 80.0\%. Thus, visibility of the correct label is not correctness: several labels can be highly ranked, and task prediction depends on their relative scores.
...
The corrected TruthfulQA profiles do not preserve the architecture ordering previously suggested by a scalar final-layer statistic. Qwen2.5-7B, Llama-3.1-8B, and Mistral-7B have close first-entry depths (.824, .833, and .788) despite different intervention outcomes in preliminary experiments. FEP should therefore not be used as an intervention-selection rule without a prospective, controlled study. We exclude FEP-guided intervention, LayerNorm-boundary, and head-at-boundary results from the present claims because their layer choices depended on the audited scalar.

\paragraph{Recommended reporting checklist.}
Layerwise vocabulary-readout studies should: (1) tokenize the full prompt and continuation jointly; (2) distinguish never observed from final-layer entry; (3) report persistence or the complete trajectory, not only first entry; (4) validate the last projected state against native logits; (5) report sensitivity to $k$; (6) separate candidate-set scores from full-vocabulary ranks; and (7) name the projection explicitly and avoid causal or storage language unless independently tested.
...
\section*{Limitations}
We analyze one raw logit-lens projection, one prompt template per task, English benchmarks, base models, and only the first continuation token. The results do not establish how trained probes, alternative prompts, instruction tuning, multilingual inputs, or multi-token generation change the trajectories. Conditional means describe only observed or final-present subsets and should not be compared without their censoring rates. Our model comparisons are observational because architecture, data, optimization, tokenizer, and scale vary jointly. The MMLU sample is balanced over 24 subjects but is not the full benchmark. Finally, rank thresholds discard score margins; future work should jointly model rank, probability, and candidate-relative evidence.
...
The supplementary artifact includes the exact runner, pure measurement helpers, unit tests, compressed per-sample JSON, aggregate CSV tables, figure-generation code, environment versions, and SHA-256 checksums. All random sampling uses seed 42. Canonical files record model references, dtype, prompt, aligned continuation IDs, target IDs, ranks and probabilities for every layer, candidate predictions where applicable, final-logit audit fields, and failures.
