...
\begin{itemize}
    ...
    \item evidence that first visibility, persistence, and task competence are distinct, that pairwise readout-depth orderings can reverse with target composition, that rank entry carries little probability mass, and a minimal reporting checklist intended to prevent over-interpretation.
\end{itemize}
...
\subsection{Target-frequency stratification}
\label{sec:frequency-strata}
Because 63.8\% of targets fall in ten lexical types, cross-model depth differences could reflect target composition rather than the model. The paired contrasts persist inside strata: exact sign tests on first-entry depth survive Holm correction for 13/15 pairs among frequent types and 12/15 pairs in the tail, with the same counts whether Holm is applied within each stratum or across all 30 tests. For Gemma versus Pythia the stratified means are .790 versus .334 ($n{=}424$, adjusted $p<10^{-84}$) and .989 versus .776 ($n{=}143$, adjusted $p<10^{-27}$). The path differences are therefore not a first-token composition artifact.

The pooled ranking, however, can invert. For 7 of 15 pairs the sign of the depth difference reverses between strata, and both directions are individually significant after Holm correction. Mistral enters .472 later than Pythia on frequent types but .027 earlier in the tail; Llama enters .051 later than Gemma on frequent types and .170 earlier in the tail. Which model reads out earlier is thus a property of the target population as well as the model, and a single pooled depth summarizes neither stratum. The count is 7/15 at frequency cuts of 5, 10, and 20 types and 8/15 at 30, so the qualitative conclusion is insensitive to this cut (Appendix~\ref{app:cuts}).
...
\toprule
Model & Freq. $F$ & Tail $F$ & $\Delta$ & Freq. $N$ & Tail $N$ \\
\midrule
Llama-3.1-8B & .836 & .827 & $+$.009 & 22.3 & 33.1 \\
Mistral-7B & .793 & .778 & $+$.015 & 24.1 & 31.5 \\
Qwen2.5-14B & .867 & .882 & $-$.015 & 27.1 & 31.8 \\
Qwen2.5-7B & .769 & .939 & $-$.170 & 26.7 & 38.9 \\
Gemma-7B & .793 & .988 & $-$.195 & 14.4 & 39.5 \\
Pythia-6.9B & .335 & .783 & $-$.448 & 10.0 & 42.9 \\
\bottomrule
\end{tabular}
}
\caption{First-entry depth $F$ and non-entry rate $N$ (percent) for targets whose lexical first token is among a model's ten most frequent types (521--522 items) versus the tail (295--296 items). $\Delta$ is the depth difference. Readout depth is frequency-sensitive for three models and nearly flat for three others.}
\label{tab:frequency-strata}
\end{table}

Depth itself is also frequency-sensitive, and unevenly so (Table~\ref{tab:frequency-strata}). Llama, Mistral, and Qwen2.5-14B differ by at most .015 between strata, whereas Qwen2.5-7B, Gemma, and Pythia differ by .170--.448. Pythia's headline .454 is thus a mixture: .335 over frequent types and .783 in the tail, where it is close to Mistral's .778. Non-entry rates move in the same direction, from 10.0\% to 42.9\% for Pythia. Reporting a single conditional depth without the target-composition profile can therefore attribute a target-population effect to a model's internal timing---another way in which the measurement describes an interface. Any-gap rates split the same way: 72.7\% versus 4.4\% for Gemma, 37.0\% versus 13.2\% for Pythia, and 25.7\% versus 6.8\% for Qwen2.5-7B, against differences of at most 4.1 points for the other three models. Targets that enter earlier are exposed to more subsequent readouts, so Gemma's penultimate-layer collapse falls almost entirely on frequent types.
...
\bottomrule
\end{tabular}
\caption{Threshold sensitivity for Qwen2.5-7B on TruthfulQA (percent except normalized depths). Conclusions depend quantitatively on $k$, so a single threshold should not be presented as intrinsic. The same sweep for all six models is released.}
\label{tab:topk}
\end{table}

Increasing $k$ smoothly reduces censoring and shifts entry earlier (Table~\ref{tab:topk}); there is no threshold-independent ``emergence layer.'' Even at $k=100$, however, observed targets first enter at 71.0\% depth and stabilize at 80.6\% depth on average. We repeat the sweep for all six models (Appendix~\ref{app:topk}). Its direction is uniform: from $k{=}1$ to $k{=}100$, non-entry falls by 50.2--65.7 points and conditional first entry moves 0.122--0.263 earlier. The extrema are preserved at every threshold: Pythia remains earliest, and Qwen2.5-14B remains latest or tied for latest. The appropriate conclusion is therefore conditional: absolute depths and intermediate-model comparisons depend on $k$, even though these endpoint comparisons do not.

\subsection{Rank entry is not probability mass}
\label{sec:probability}
A rank threshold treats top-10 membership as visibility, but in vocabularies of 32k--248k tokens that membership is compatible with almost no probability mass. At the rank-based first entry the target's median probability is only .005--.018, and 25.6--72.1\% of observed entries occur below .01 (Table~\ref{tab:probability}). The event that the audited statistic records is therefore weaker than the word ``visible'' suggests.

Replacing the criterion changes both the timing and the population. Requiring $p\ge.05$ at some layer reduces the share of examples that ever qualify from 68.9--78.1\% to 46.3--55.8\%: about half of all targets never reach five percent probability at any layer, although most of them enter the top-10. On the 378--456 examples per model that satisfy both criteria, the probability criterion fires .042--.227 later in normalized depth, so the two events are not interchangeable even where both occur. Raising the floor to $.10$ preserves this conclusion: coverage falls to 36.0--46.9\%, and the paired delay remains positive at .055--.288. The shift is again unequal across models. Pythia's $+.227$ at the $.05$ floor is 1.8 times the next largest and 5.4 times the smallest, so the model with the earliest rank-based entry is also the one whose entry carries the least mass (median .005, with 72.1\% of entries below .01); it remains the earliest model under that probability criterion. We report the target's own probability only, because the artifacts do not store the per-layer arg-max probability, so we make no claim about the margin to the top-ranked token.

\begin{table}[t]
\centering
\resizebox{\columnwidth}{!}{%
\input{tables/probability_criterion}
}
\caption{Rank criterion versus probability criterion on 817 TruthfulQA examples. Median $p$ is the median target probability at the rank-based first entry, and the next column gives the percentage of those entries below one percent. Cover is the percentage of all examples that ever reach probability $.05$. Paired $\Delta$ is the mean depth difference between the two criteria on the examples that satisfy both, so it is a within-example comparison. Rows are ordered by $\Delta$.}
...
The two neutral templates are token-matched---both average 16.93 prompt tokens---and give similar results (paired exact McNemar $p=.54$ for final visibility), so relabelling the turn markers alone does not move readout. Prepending ``Answer the following question truthfully'' adds seven tokens and increases final top-10 visibility by 9.7 points ($110$ gains vs. $31$ losses, $p<10^{-10}$), while also shifting persistent entry earlier (Table~\ref{tab:prompts}). The model, questions, targets, and threshold are held fixed. Vocabulary readout is therefore not a model-only profile; an added instruction sentence can materially reshape it. We do not separate the instruction's semantics from its length, which would require a length-matched neutral prefix.
...
Layerwise vocabulary-readout studies should: (1) tokenize the full prompt and continuation jointly; (2) distinguish never observed from final-layer entry; (3) report persistence or the complete trajectory, not only first entry; (4) distinguish any intermediate gap from final disappearance; (5) validate the last projected state against native logits; (6) report sensitivity to $k$ and influential individual readouts; (7) separate candidate-set scores from full-vocabulary ranks; (8) pair the rank criterion with a probability criterion, because top-$k$ membership can carry little mass; and (9) name the projection explicitly and avoid causal or storage language unless independently tested.

\section{Conclusion}
Layerwise vocabulary projections are informative, but their simplest event summary can be misleading. Across six language models, censoring and non-monotonicity are common, and models with similar final visibility follow qualitatively different paths. For 7 of 15 model pairs, which model reads out earlier reverses with the target population, so a pooled depth ranking summarizes neither stratum. Entry is also a weak event: at the moment a target enters the top-10 its median probability is only .005--.018, and a probability criterion fires later on the same examples. Across task formats, candidate competence can be nearly independent of full-vocabulary top-$k$ visibility. A trajectory-aware, censoring-aware protocol turns these apparent contradictions into measurable distinctions. The broader lesson is methodological: vocabulary readout should be treated as an interface-sensitive observation, not as a timestamp for knowledge itself.

\section*{Limitations}
We analyze one raw logit-lens projection, one prompt template per task, English benchmarks, base models, and only the first continuation token. On TruthfulQA, 97.9--99.1\% of reference answers are multi-token and the first-token distribution is concentrated, so our evidence concerns answer-prefix readout rather than full factual answers; Table~\ref{tab:frequency-strata} quantifies how much depth depends on that distribution. The results do not establish how trained probes, alternative prompts, instruction tuning, multilingual inputs, or multi-token generation change the trajectories. Gemma's single-readout collapse shows that raw-projection summaries can be sensitive to a local representation--unembedding mismatch; leave-one-readout-out analysis diagnoses but does not explain or repair that mismatch. Conditional means describe only observed or final-present subsets and should not be compared without their censoring rates. Our model comparisons are observational because architecture, data, optimization, tokenizer, and scale vary jointly. Pairwise tests are post-hoc and correction-dependent. The frequency strata use each model's ten most frequent first-token types, matching the target audit; we release a sweep over that cut, and frequency is measured within this benchmark rather than in pretraining data. The MMLU sample is balanced over 24 subjects but is not the full benchmark. The historical GPU runs recorded local model snapshot paths but not immutable hub revisions, limiting bitwise rerun provenance; all per-sample artifacts and checksums are retained. Finally, Section~\ref{sec:probability} pairs the rank criterion with a probability criterion, but we do not model candidate-relative evidence or the margin to the arg-max token, which the artifacts do not record.
...
The supplementary artifact includes the runner, pure measurement helpers, unit tests, compressed per-sample JSON, aggregate CSV tables, figure-generation code, command manifest, environment versions, and SHA-256 checksums. All random sampling uses seed 42. Canonical files record local model references, dtype, prompts, aligned continuation IDs, target IDs, ranks and probabilities for every layer, candidate predictions where applicable, final-logit audit fields, and failures. New runs additionally record requested and resolved model/tokenizer revisions and dataset checksums or revisions; the runs reported here predate that instrumentation. The original runs did not capture immutable model revisions; we disclose rather than reconstruct this missing provenance.
...
\label{app:topk}
Table~\ref{tab:app-topk} gives the $k$ sweep for all six models; Table~\ref{tab:topk} prints one column of it. Non-entry falls monotonically with $k$ for every model and conditional first entry moves earlier. Intermediate models do change order, but the extrema do not: Pythia is earliest and Qwen2.5-14B latest or tied for latest at every threshold.

\begin{table*}[t]
\centering
\input{tables/appendix_topk}
\caption{Threshold sweep on 817 TruthfulQA examples for all six models. Depths are conditional on ever-observed targets.}
\label{tab:app-topk}
\end{table*}

\section{Complete Pairwise Comparisons}
\label{app:pairwise}
Tables~\ref{tab:app-visibility} and~\ref{tab:app-depth} list every model pair behind the counts in Section~\ref{sec:truthfulqa}. All tests are exact and post hoc. The visibility tests pair all 817 items; the depth tests pair only items observed by both models, so their sample sizes range from 507 to 567.

\begin{table*}[t]
\centering
\resizebox{\textwidth}{!}{%
\input{tables/appendix_pairwise_visibility}
}
\caption{Paired exact McNemar tests on final top-10 visibility. ``A only'' counts items visible for model A and not model B. Holm gives 3/15 rejections at $.05$ and Benjamini--Hochberg gives 9/15.}
\label{tab:app-visibility}
\end{table*}

\begin{table*}[t]
\centering
\resizebox{\textwidth}{!}{%
\input{tables/appendix_pairwise_depth}
}
\caption{Paired exact sign tests on first-entry depth, restricted to items observed by both models. $n$ is the pooled jointly observed count; the stratified counts are in the released table. $\Delta$ is the mean depth of model A minus model B, so a positive value means model A reads out later. Holm correction is applied within each of the three families. A dagger marks the seven pairs whose sign reverses between strata with both directions significant.}
\label{tab:app-depth}
\end{table*}

\section{Stratum-Cut Sensitivity}
\label{app:cuts}
The frequency strata in Section~\ref{sec:frequency-strata} use each model's ten most frequent first-token types. Table~\ref{tab:app-cuts} recomputes the stratified tests at four cuts. The reversal count changes only from 7 to 8 of 15 pairs, so the qualitative conclusion is insensitive to the reported cut.

\begin{table}[!ht]
\centering
\resizebox{\columnwidth}{!}{%
\input{tables/appendix_cut_sensitivity}
}
\caption{Stratified tests recomputed at first-token frequency cuts of 5, 10, 20, and 30 types.}
\label{tab:app-cuts}
\end{table}
